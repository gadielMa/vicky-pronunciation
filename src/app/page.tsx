...
function Hero() {
  return <section className="relative overflow-hidden px-5 pb-20 pt-16 md:px-10 md:pb-28 md:pt-24">
    <div className="absolute -right-24 -top-32 size-[28rem] rounded-full bg-[var(--purple-light)] opacity-70 blur-3xl" /><div className="absolute -bottom-40 -left-32 size-[26rem] rounded-full bg-[var(--mint-light)] opacity-80 blur-3xl" />
    <div className="relative mx-auto max-w-6xl"><div className="grid items-end gap-12 lg:grid-cols-[1.25fr_0.75fr]"><div>
      <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[rgba(122,79,196,0.16)] bg-white px-3.5 py-1.5 text-xs font-bold uppercase tracking-[0.12em] text-[var(--purple-dark)]"><Sparkles className="size-3.5" aria-hidden /> Argentine Spanish, with Victoria</div>
...
function Offerings() {
  return <section id="sessions" className="bg-white px-5 py-20 md:px-10 md:py-28"><div className="mx-auto max-w-6xl"><div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[0.16em] text-[var(--mint-darkest)]">Four ways to learn</p><h2 className="mt-3 text-4xl font-black tracking-[-0.055em] text-[var(--navy)] md:text-5xl">Choose how you want Spanish to show up in your life.</h2></div><div className="mt-12 grid gap-4 md:grid-cols-2">
    {offerings.map(({ number, icon: Icon, title, eyebrow, hook, body, details, color, accent }) => <article key={title} className="group relative overflow-hidden rounded-[1.75rem] border border-[rgba(71,67,80,0.08)] p-7 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_22px_45px_rgba(61,37,112,0.16)] md:p-8" style={{ background: color }}><div aria-hidden className="pointer-events-none absolute -right-12 -top-12 size-36 rounded-full bg-white/35 blur-2xl transition-transform duration-700 group-hover:scale-150 animate-float" /><div className="relative"><div className="flex items-start justify-between gap-4"><div className="flex size-11 items-center justify-center rounded-2xl bg-white shadow-sm transition-transform duration-500 group-hover:-rotate-6 group-hover:scale-110" style={{ color: accent }}><Icon className="size-5" aria-hidden /></div><span className="text-xs font-black tracking-[0.16em] transition-transform duration-500 group-hover:translate-x-1" style={{ color: accent }}>{number}</span></div><p className="mt-8 text-xs font-bold uppercase tracking-[0.13em]" style={{ color: accent }}>{eyebrow}</p><h3 className="mt-2 text-3xl font-black tracking-[-0.045em] text-[var(--navy)]">{title}</h3><p className="mt-4 text-lg font-bold leading-snug text-[var(--navy)]">{hook}</p><p className="mt-4 text-sm leading-relaxed text-[var(--navy-light)]">{body}</p><ul className="mt-6 space-y-2.5 border-t border-[rgba(71,67,80,0.1)] pt-5">{details.map((detail) => <li key={detail} className="flex gap-2.5 text-sm text-[var(--navy)]"><Check className="mt-0.5 size-4 shrink-0" style={{ color: accent }} aria-hidden />{detail}</li>)}</ul><a href="#contact" className="mt-7 inline-flex items-center gap-1 text-sm font-extrabold transition-all duration-300 group-hover:gap-2" style={{ color: accent }}>Ask about this format <ChevronRight className="size-4" aria-hidden /></a></div></article>)}
  </div></div></section>;
}
...
function FamilyApp() {
  return <section id="family-app" className="bg-[var(--cream)] px-5 py-20 md:px-10 md:py-28"><div className="mx-auto grid max-w-6xl items-center gap-10 rounded-[2rem] bg-white px-7 py-12 shadow-[0_14px_50px_rgba(71,67,80,0.07)] md:grid-cols-[1.15fr_0.85fr] md:px-14 md:py-16"><div><div className="inline-flex items-center gap-2 rounded-full bg-[var(--purple-light)] px-3.5 py-1.5 text-xs font-bold uppercase tracking-[0.12em] text-[var(--purple-dark)]"><Sparkles className="size-3.5" aria-hidden /> The family app</div><h2 className="mt-6 max-w-xl text-4xl font-extrabold leading-[1.05] tracking-[-0.045em] text-[var(--navy)] md:text-5xl">Your family&apos;s bridge to <span className="text-[var(--purple-dark)]">Argentina</span>—through language, culture &amp; play.</h2><p className="mt-6 max-w-xl text-lg leading-relaxed text-[var(--navy-light)]">A gentler space for children and bilingual families: interactive Argentine Spanish lessons, cultural activities and playful ways to keep the language close to home.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/lessons/saludos-argentinos" className="rounded-full bg-[var(--purple-dark)] px-5 py-3 text-sm font-bold text-white transition hover:bg-[var(--purple-darkest)]">Explore a sample lesson</Link><Link href="/families" className="rounded-full border border-[var(--purple)] px-5 py-3 text-sm font-bold text-[var(--purple-dark)] transition hover:bg-[var(--purple-light)]">Open the family app</Link></div></div><div className="rounded-[1.75rem] bg-[var(--mint-light)] p-7"><p className="text-sm font-bold text-[var(--mint-darkest)]">Made for quieter learning moments.</p><div className="mt-6 space-y-3"><div className="rounded-2xl bg-white p-4 text-sm font-semibold text-[var(--navy)]">Interactive lessons for children</div><div className="rounded-2xl bg-white p-4 text-sm font-semibold text-[var(--navy)]">Argentine culture through play</div><div className="rounded-2xl bg-white p-4 text-sm font-semibold text-[var(--navy)]">A familiar language kept close</div></div><p className="mt-6 text-xs leading-relaxed text-[var(--mint-darkest)]">The app is currently a quieter, supporting part of elSur while Victoria&apos;s sessions take centre stage.</p></div></div></section>;
}

function Sobremesa() {
  return <section className="bg-[var(--cream)] px-5 py-16 md:px-10"><div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 rounded-[1.75rem] border border-[rgba(71,67,80,0.1)] bg-white p-7 md:flex-row md:items-center md:p-9"><div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[0.15em] text-[var(--blush-dark)]">Special series</p><h2 className="mt-2 text-3xl font-black tracking-[-0.05em] text-[var(--navy)]">Sobremesa Sessions</h2><p className="mt-3 leading-relaxed text-[var(--navy-light)]">Literature, current affairs and debate in Spanish. For people who enjoy a conversation that lingers after the plates are cleared.</p></div><a href="#contact" className="shrink-0 rounded-full border border-[var(--blush-dark)] px-5 py-3 text-sm font-bold text-[var(--blush-dark)] transition hover:bg-[var(--blush)]">Keep me posted</a></div></section>;
}
